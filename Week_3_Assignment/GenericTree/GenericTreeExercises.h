...
static void treeFactory(GenericTree<int>& tree) {
  auto root_node = tree.getRootPtr(); //create root node
  /*step 1: Deallocate the entire tree if there is data*/
  if (root_node != nullptr) {
    tree.clear();
    tree.compress(); //remove the nullptrs in the subtree 
  }

  tree.createRoot(4); //create root to be 4 
  auto node_8 = root_node -> addChild(8); 
  auto node_4 = root_node -> addChild(15); 
  auto node_16 = node_8 -> addChild(16); 
  auto node_23 = node_8 -> addChild(23); 
  auto node_42 = node_16 -> addChild(42); 
  
  //test by printing tree 
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while(!Q.empty()) { //while there is at least one discovered node 
    for (auto childPtr : Q.front()->childrenPtrs) { //if there are nodes to explore 
      Q.push(childPtr); } 
      results.push_back(Q.front()->data); //add data to standard vector 
      //push addresses of children into queue 
      Q.pop(); 
    }
